        ...
        <div class="row wrapper white-bg page-heading">
                ...
                <div class="row">
                  <div class="col-md-12 text-xs-right">
                    <div class="btn-group mt-2">
                      <button type="button" class="btn btn-primary btn-min-width dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="icon-print"></i> Print Purchase</button>
                      <div class="dropdown-menu">
                        <a class="dropdown-item" href="/AIS/purchase-print?id=<?= $purchase['id'] ?>" target="_blank">Print</a>
          ...
          <div id="invoice-customer-details" class="row pt-2">
            <div class="col-md-4">
              ...
              <ul class="list-unstyled">
                <li class="text-bold-800"><a
                                            href="/AIS/supplier-view?id=<?= $purchase['supplier_id']  ?>"><strong
            ...
            <div class="col-md-4">
              ...
              <p>Due Date: <?= date('d-m-Y', strtotime($purchase['due_date'])) ?></p>
          ...
          <div id="invoice-items-details" class="pt-2">
            ...
            <div class="row mt-3">
              <div class="col-md-7">
                <p class="lead">Payment Status: <strong><?= $purchase['payment_status'] ?? 'Unpaid' ?></strong></p>
                <p class="lead">Payment Method: <strong><?= $purchase['payment_method'] ?? '---' ?></strong></p>
                <p class="lead mt-1">Note:</p>
                <code><?= $purchase['notes'] ?? 'No notes provided.' ?></code>
              </div>
              <div class="col-md-5">
                ...
                <table class="table">
                 <tr><td>Sub Total</td><td class="text-xs-right"><?= $currencySymbol ?> <?= number_format($item['subtotal'], 2) ?></td></tr>
                  ...
                  <tr><td>Shipping</td><td class="text-xs-right"><?= $currencySymbol ?> <?= number_format($purchase['shipping'], 2) ?></td></tr>
                  <tr class="bg-light"><td class="text-bold-800">Total</td><td class="text-bold-800 text-xs-right"><?= $currencySymbol ?> <?= number_format($purchase['grand_total'], 2) ?></td></tr>
                  <tr><td>Payment Made</td><td class="text-xs-right"><?= $currencySymbol ?> <?= number_format($payment['amount'] ?? 0, 2) ?></td></tr>
                  <tr class="bg-grey bg-lighten-4"><td class="text-bold-800">Balance Due</td><td class="text-bold-800 text-xs-right"><?= $currencySymbol ?> <?= number_format(($purchase['grand_total'] - ($payment['amount'] ?? 0)), 2) ?></td></tr>
                </table>
        ...
        <div id="invoice-footer">
            ...
            <div class="row">
              <div class="col-md-7">
                ...
                <p><?= nl2br($purchase['terms'] ?? 'Payment Due On Receipt.') ?></p>
              </div>
            </div>
        </div>
