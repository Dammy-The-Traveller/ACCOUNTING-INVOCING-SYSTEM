...
$purchase = $purchase ?? [];  // Ensures $purchase is at least an array
?>
...
<?php endif; ?>
<?php if(isset($purchase) && $isEdit){
if($purchase['currency'] == 1){
      $currencySymbol = '£';
   ...
   <div class="content-wrapper">
     <div class="content-header row">
       <div class="card card-block">
         ...
         <form method="post" id="data_form" action="<?= $isEdit ? '/AIS/purchase-update' : '/AIS/purchase-store' ?>">
          <input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?>">
           <input type="hidden" name="id" value="<?= $isEdit ? $purchase['id'] : '' ?>">
           <div class="row">
             <div class="col-sm-4">
             </div>
               ...
               <div id="customerpanel" class="inner-cmp-pnl">
                 ...
                 <div id="customer">
                   <div class="clientinfo">
                     Supplier Details
                     <hr>
                     <input type="hidden" name="supplier_id" id="supplier_id" value="<?= $isEdit ? ($purchase['supplier_id'] ?? '0') : '0'  ?>">
                   ...
                   <select id="warehouses" name="warehouses" class="selectpicker form-control">
                     ...
                     <?php foreach ($warehouses as $warehouse): ?>
                                ...
                                <?= ($isEdit && $warehouse['id'] == $purchase['warehouse_id']) ? 'selected' : '' ?>>
                   ...
                   <select id="category" name="category" class="selectpicker form-control">
                     ...
                     <?php foreach ($categories as $category): ?>
                                ...
                                <?= $isEdit && $category['id'] == $purchase['category_id'] ? 'selected' : '' ?>
               ...
               <div class="inner-cmp-pnl">
                 ...
                 <div class="form-group row">
                   <div class="col-sm-6"><label for="invocieno" class="caption">Invoice No</label>
                     <div class="input-group">
                       ...
                       <input type="text" class="form-control" placeholder="Invoice #" name="invocieno" value="<?= $isEdit? $purchase['invoice_no'] :  generateInvoiceIndex() ?>">
                     </div>
                   </div>
                   <div class="col-sm-6"><label for="invocieno" class="caption">Reference </label>
                     <div class="input-group">
                       <div class="input-group-addon"><span class="icon-bookmark-o" aria-hidden="true"></span></div>
                       <input type="text" class="form-control" placeholder="Reference #" name="refer" value="<?= $isEdit ? $purchase['reference'] : '' ?>">
                     </div>
                   </div>
                   ...
                   <div class="col-sm-6"><label for="invociedate" class="caption">Order Date </label>
                     <div class="input-group">
                       <div class="input-group-addon"><span class="icon-calendar4" aria-hidden="true"></span></div>
                                <input required type="text" class="form-control required" placeholder="Billing Date" name="invoicedate" data-toggle="datepicker" autocomplete="false" value="<?= $isEdit ? date('Y-m-d', strtotime($purchase['invoice_date'])) : date('d-m-Y') ?>" id="sdate">
                     </div>
                   </div>
                   <div class="col-sm-6"><label for="invocieduedate" class="caption">Order Due Date </label>
                     <div class="input-group">
                       <div class="input-group-addon"><span class="icon-calendar-o" aria-hidden="true"></span></div>
                         ...
                         <input required type="text" class="form-control required"  name="invocieduedate" placeholder="Due Date" data-toggle="datepicker" autocomplete="false" value="<?= $isEdit ? date('d-m-Y', strtotime($purchase['due_date'])) : date('d-m-Y') ?>" id="sdate">
                     </div>
                   </div>
                 </div>
                 <div class="form-group row">
                   <div class="col-sm-6">
                     ...
                     <select class="form-control" name="taxformat" onchange="changeTaxFormat(this.value)" id="taxformat">
                       ...
                       <option value="on" <?= $isEdit && $purchase['tax_format'] == 'on' ? 'selected' : '' ?>>On</option>
                       <option value="off" <?= $isEdit && $purchase['tax_format'] == 'off' ? 'selected' : '' ?>>Off</option>
                     </select>
                   </div>
                   <div class="col-sm-6">
                     <div class="form-group">
                       ...
                       <select class="form-control" name="discountFormat" onchange="changeDiscountFormat(this.value)" id="discountFormat">
                          <option value="%" <?= $isEdit && $purchase['tax_format'] == '%' ? 'selected' : '' ?>> % Discount After TAX </option>
                 <option value="flat" <?= $isEdit && $purchase['tax_format'] == 'flat' ? 'selected' : '' ?>>Flat Discount After TAX</option>
                 <option value="b_p" <?= $isEdit && $purchase['tax_format'] == 'b_p' ? 'selected' : '' ?>> % Discount Before TAX</option>
                 <option value="bflat" <?= $isEdit && $purchase['tax_format'] == 'bflat' ? 'selected' : '' ?>>Flat Discount Before TAX</option>
                 ...
                 <div class="form-group row">
                   <div class="col-sm-12">
                     ...
                     <textarea class="form-control" name="notes" rows="2" ><?=  $isEdit ? ($purchase['notes'] ?? '') : ''  ?></textarea>
                   </div>
                 </div>
               </div>
             </div>
...
<input type="hidden" name="disca[]" id="disca-<?= $isEdit ?  $index : '' ?>" 
       value="<?= $isEdit ? $item['discount_amount'] : '' ?>">
...
<input type="hidden" class="pdIn" name="pid[]" id="pid-<?= $isEdit ?  $index : ''?>" 
        ...
        <textarea id="dpid-<?=$isEdit ?  $index : ''?>" class="form-control" name="product_description[]"><?= $isEdit ?  $item['product_description'] : ''?></textarea><br>
           ...
           <td colspan="8"><textarea id="dpid-0" class="form-control" name="product_description[]" placeholder="Enter Product description" autocomplete="off"></textarea><br></td>
                 ...
                 <tr class="last-item-row">
                   <td class="add-row">
                     <button type="button" class="btn btn-success" aria-label="Left Align" data-toggle="tooltip" data-placement="top" title="Add product row" id="addproductss">
  ...
  <td colspan="6" align="right">
    <input type="hidden" value="<?= $isEdit ? ($purchase['subtotal'] ?? '0') : '0' ?>" id="subttlform" name="subtotal">
    <strong> Total Tax</strong>
  </td>
  <td align="left" colspan="2">
    <span class="currenty lightMode"><?= $isEdit ? ($currencySymbol ?? '') : '' ?></span>
    <span id="taxr" class="lightMode"><?= $isEdit ? (array_sum(array_column($items, 'tax_amount'))?? '0') : '0' ?></span>
  </td>
</tr>
...
<tr class="sub_c" style="display: table-row;">
  ...
  <td align="left" colspan="2">
    <span class="currenty lightMode"><?= $isEdit ? ($currencySymbol ?? '') : '' ?></span>
    <span id="discs" class="lightMode"><?= $isEdit ? (array_sum(array_column($items, 'discount_amount')) ?? '0') : '0' ?></span>
  </td>
</tr>
...
<tr class="sub_c" style="display: table-row;">
  ...
  <td align="left" colspan="2">
    <input type="text" class="form-control shipVal" onkeypress="return isNumber(event)" placeholder="Value" name="shipping" autocomplete="off" onkeyup="updateTotal()" value="<?= $isEdit ? ($purchase['shipping'] ?? '0') : '0' ?>">
  </td>
</tr>
...
<tr class="sub_c" style="display: table-row;">
  ...
  <td align="left" colspan="2">
    <input type="text" name="total" class="form-control" id="invoiceyoghtml" readonly="" value="<?= $isEdit ? ($purchase['grand_total'] ?? '0') : '0' ?>">
  </td>
</tr>
...
<tr class="sub_c" style="display: table-row;">
  <td colspan="2">
    ...
    <select name="pterms" class="selectpicker form-control">
      <option value="1" <?= $isEdit ? (($purchase['payment_terms'] ?? '') == 1 ? 'selected' : '') : '' ?>>Due On Receipt</option>
      <option value="2" <?= $isEdit ? (($purchase['payment_terms'] ?? '') == 2 ? 'selected' : '') : '' ?>>Net 15</option>
      <option value="3" <?= $isEdit ? (($purchase['payment_terms'] ?? '') == 3 ? 'selected' : '') : '' ?>>Net 30</option>
    </select>
  </td>
  <td colspan="2">
    <div>
      ...
      <div class="input-group">
        <label class="display-inline-block custom-control custom-radio ml-1">
          <input type="radio" name="update_stock" class="custom-control-input" value="yes" <?= $isEdit ? (($purchase['update_stock'] ?? '') == 'yes' ? 'checked' : '') : '' ?>>
          <span class="custom-control-indicator"></span>
          <span class="custom-control-description ml-0">Yes</span>
        </label>
        <label class="display-inline-block custom-control custom-radio">
          <input type="radio" name="update_stock" class="custom-control-input" value="no" <?= $isEdit ? (($purchase['update_stock'] ?? '') == 'no' ? 'checked' : '') : '' ?>>
          <span class="custom-control-indicator"></span>
...
document.addEventListener("DOMContentLoaded", () => {
    let rowCount = <?= $isEdit ? count($items) : 1 ?>;
    document.getElementById("addproductss").addEventListener("click", function () {
        ...
        const rowHTML = `
            <tr>
                <td><input type="text" class="form-control text-center product-search" name="product_name[]" id="productname-${rowId}" placeholder="Enter Product name or Code" aria-label="Product Name"></td>
...
<script>
document.addEventListener("DOMContentLoaded", () => {
    // Event delegation for all product name inputs (current and future)
    document.querySelector(".tfr tbody").addEventListener("input", function (e) {
        if (!e.target || !e.target.matches(".product-search")) return;
        ...
        if (query.length < 2) {
            ...
            if (!warehouseId || warehouseId === "0" || !categoryId || categoryId === "0") {
        ...
        fetch(`/AIS/api/product_details?query=${encodeURIComponent(query)}&warehouse_id=${warehouseId}&category_id=${categoryId}`)
            ...
            .then(data => {
                ...
                data.forEach(product => {
                    const div = document.createElement("div");
                    div.classList.add("product-suggestion", "p-2", "border-bottom", "cursor-pointer");
